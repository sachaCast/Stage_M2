    ...
    public class PointCutBean
    {
        ...
        public string AADefinition
        {
            get
            {
                ...
                {
                    string errTimeout = PointCutBean.PARSE_ERROR;
                    Logger.Warn($"[SORTIE] AADefinition (get) - Timeout Mutex. Retour : {errTimeout}");
                ...
                if (this.AA != null)
                {
                    ...
                    Logger.Info($"[SORTIE] AADefinition (get) - Définition de l'aspect extraite : {aaDefinition}");
                    return aaDefinition;
                }
                this.parserMutex.ReleaseMutex();
                string finalError = this.error != null ? this.error : PointCutBean.PARSE_ERROR;
                Logger.Warn($"[SORTIE] AADefinition (get) - Erreur retournée : {finalError}");
                return this.error != null ? this.error : PointCutBean.PARSE_ERROR;
            }
            set
            {
                Logger.Info($"[ENTRÉE] AADefinition (set) - Reçu : '{value}'");
        ...
        private AspectOfAssembly GetAAFromDefinition(string definition)
        {
            ...
            if (aaParser.Parse())
            {
                Logger.Info($"[SORTIE INTERNE] GetAAFromDefinition - Aspect analysé avec succès : '{aaParser.ParsedAA?.Name}'");
                return aaParser.ParsedAA;
            }
            this.error = $"{PointCutBean.PARSE_ERROR}{aaScanner.strError}";
            Logger.Error($"[SORTIE INTERNE] GetAAFromDefinition - Échec du parsing. Erreur : {this.error}");
            return (AspectOfAssembly)null;
        }

        public void SetApplicationAssembly(List<VariableComponent> beanList)
        {
            int currentCount = beanList != null ? beanList.Count : 0;
            Logger.Info($"[ENTRÉE] SetApplicationAssembly - Reçu une liste de {currentCount} composant(s).");
            ...
            else
            {
                ...
                LogAspectOfAssembly("APRÈS MATCHING", this.AA);
        ...
        private bool FindAllMatchingJoinpoints(List<VariableComponent> lJP)
        {
            ...
            foreach (Pointcut pointcut in this.AA.Pointcuts)
            {
                ...
                foreach (VariableComponent variableComponent in lJP)
                {
                    ...
                    {
                        Logger.Info($"[SORTIE INTERNE] Liaison validée : Composant '{variableComponent.InstanceName}' intercepté par la variable '{pointcut.Variable}'");
        ...
        private void FireLListOfJoinpoints()
        {
            ...
            {
                Logger.Warn("[SORTIE ÉVÉNEMENT] Événement FireLListOfJoinpoints ignoré : Aucun abonné.");
                return;
            }
        ...
        private void LogAspectOfAssembly(string moment, AspectOfAssembly aa)
        {
            Logger.Info("=== AspectOfAssembly [" + moment + "] ===");

            if (aa == null)
            {
                Logger.Info("   AA est NULL");
                return;
            }

            // Champ : Name
            Logger.Info("   Nom : " + aa.Name);

            // Champ : Pointcuts
            Logger.Info("   Pointcuts (" + aa.Pointcuts.Count + ") :");
            foreach (Pointcut pc in aa.Pointcuts)
            {
                Logger.Info("      Pointcut :");
                Logger.Info("         Variable           : " + pc.Variable);
                Logger.Info("         SyntacticExpression: " + pc.SyntacticExpression);

                // CORRECTION LINQ ICI : Conversion du Dictionary<string, string> en chaînes "Cle=Valeur"
                string semanticText = pc.SemanticExpression != null ?
                    string.Join(", ", (string[]) pc.SemanticExpression.Select(kvp => kvp.Key + "=" + kvp.Value)) : "null";

                Logger.Info("         SemanticExpression : " + semanticText);

                Logger.Info("         Joinpoints (" + pc.Joinpoints.Count + ") :");
                foreach (VariableComponent jp in pc.Joinpoints)
                {
                    Logger.Info("            VariableName : " + jp.VariableName);
                    Logger.Info("            InstanceName : " + jp.InstanceName);
                }
            }

            // Champ : JoinpointCombinations
            Logger.Info("   JoinpointCombinations (" + aa.JoinpointCombinations.Count + ") :");
            int i = 0;
            ...
            Logger.Info("=== FIN AspectOfAssembly ===");
        }
    }
}