...
namespace WComp.AADesigner
{
    ...
    public class CycleManager
    {
        ...
        private Dictionary<string, List<string>> cachedApis = new Dictionary<string, List<string>>();
        private readonly string jsonFilePath = AppDomain.CurrentDomain.BaseDirectory + "detected_devices.json";
        ...
        public void SetCurrentAssemblyState(string beanList, string linkList)
        {
            ...
            else
            {
                foreach (string instanceName in strArray1)
                {
                    ...
                    // --- DEBUT EXTRACTION POUR APPAREILS DEJA PRESENTS ---
                    try
                    {
                        Type componentType = null;

                        // 1. On extrait le préfixe textuel de l'instance ("switch1" -> "switch", "light1" -> "light")
                        string cleanName = System.Text.RegularExpressions.Regex.Replace(instanceName, @"\d+$", "").ToLower();

                        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                        {
                            foreach (var t in assembly.GetTypes())
                            {
                                // Filtre strict : doit appartenir au namespace WComp et être un Proxy Device UPnP
                                if (t.IsClass && t.Namespace != null && t.Namespace.StartsWith("WComp"))
                                {
                                    string typeName = t.Name.ToLower();

                                    // On vérifie qu'il s'agit bien d'un proxy UPnP (présence de la méthode WComp _GetUDN)
                                    bool isUpnpProxy = t.GetMethod("_GetUDN", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance) != null;

                                    if (isUpnpProxy && (typeName == cleanName || typeName.Contains(cleanName)))
                                    {
                                        componentType = t;
                                        break;
                                    }
                        ...
                        if (componentType != null)
                        {
                            ...
                            {
                                if (method.DeclaringType == componentType)
                                {
                                    methodNames.Add(GetMethodSignature(method));
                                }
                            }

                            if (this.cachedApis.ContainsKey(instanceName))
                                this.cachedApis[instanceName] = methodNames;
                            else
                                this.cachedApis.Add(instanceName, methodNames);

                            Logger.Info($"[JSON INITIALISATION] API UPnP identifiée pour : {instanceName} ({componentType.FullName})");
                        }
                        else
                        {
                            Logger.Warn($"[JSON INITIALISATION] Proxy UPnP non trouvé pour l'instance : {instanceName}");
                        }
                    }
                    catch (Exception ex)
                    {
                        Logger.Error($"[JSON INITIALISATION] Erreur pour {instanceName} : {ex.Message}");
                    }
                    // --- FIN EXTRACTION ---
                }

                // On sauvegarde le JSON avec toutes les APIs des appareils déjà découverts
                this.SaveApisToJson();
        ...
        private void StartWeavingCycle()
        {
            ...
            else
            {
                ...
                // LOG AJOUTÉ : ce qu'on envoie au PointCutBean
                Logger.Info("=== CYCLE DE WEAVING DEMARRE ===");
                Logger.Info("=== DEVICES ENVOYES AU POINTCUTBEAN ("
                    + this.listJoinPoints.Count + ") :");
                foreach (VariableComponent vc in this.listJoinPoints)
                    Logger.Info("    -> " + vc.InstanceName);
                Logger.Info("=== FIN ENVOI DEVICES ===");
                ...
                switch (strArray[1])
                {
                    case "NEW_BEAN":
                        ...
                        if (!this.wovenRules.Exists((Predicate<AdviceRule>)(r => r is CompRule && ((CompRule)r).Component.InstanceName == instName)))
                        {
                            ...
                            // LOG AJOUTÉ : état de listJoinPoints après ajout
                            Logger.Info("=== DEVICE DETECTE : " + instName + " (type=" + type + ")");
                            Logger.Info("=== LISTE COMPLETE DES DEVICES CONNUS ("
                                + this.listJoinPoints.Count + ") :");
                            foreach (VariableComponent vc in this.listJoinPoints)
                                Logger.Info("    - " + vc.InstanceName);
                            Logger.Info("=== FIN LISTE DEVICES ===");

                            try
                            {
                                Logger.Info($"[Debug API] Tentative de résolution du type : {type}");
                                ...
                                if (componentType == null)
                                {
                                    foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                                    {
                                        componentType = assembly.GetType(type);
                                        if (componentType != null) break;
                                    }
                                }
                                ...
                                if (componentType != null)
                                {
                                    Logger.Info($"=== API extraite pour {instName} ===");
                                    List<string> methodNames = new List<string>();
                                    ...
                                    foreach (var method in methods)
                                    {
                                        if (method.DeclaringType == componentType)
                                        {
                                            // Au lieu de : methodNames.Add(method.Name);
                                            string signature = GetMethodSignature(method);
                                            methodNames.Add(signature);

                                            Logger.Info($"   -> [Action/Signature] {signature}");
                                        }
                                    }
                                    // Stockage en mémoire (ajoute ou écrase si déjà présent)
                                    if (this.cachedApis.ContainsKey(instName))
                                        this.cachedApis[instName] = methodNames;
                                    else
                                        this.cachedApis.Add(instName, methodNames);
                                ...
                                {
                                    Logger.Error($"[Erreur API] Le type '{type}' n'a pu être trouvé.");
                                }
                            }
                            catch (Exception ex)
                            {
                                Logger.Error("=== EXCEPTION DANS LE LOG D'API ===");
                                Logger.Error(ex.Message);
                            }
                            this.StartWeavingCycle();
                        }
                    ...
                    case "DEL_BEAN":
                        ...
                        if (this.cachedApis.ContainsKey(instName))
                        {
                            this.cachedApis.Remove(instName);
                            Logger.Info($"[JSON SYSTEM] {instName} a quitté le réseau. Suppression de son API du JSON.");
                            this.SaveApisToJson();
                        }
                        if (this.cycleRunning)
                            this.FireCancelCycle(true);
        ...
        private void SaveApisToJson()
        {
            try
            {
                List<string> jsonEntries = new List<string>();
                foreach (var pair in this.cachedApis)
                {
                    List<string> currentDeviceMethods = new List<string>();
                    foreach (var method in pair.Value)
                    {
                        currentDeviceMethods.Add($"      \"{method}\"");
                    }

                    // Correction CS1503 : On convertit la liste en tableau (.ToArray())
                    string methodsArray = string.Join(",\r\n", currentDeviceMethods.ToArray());
                    string entry = $"  \"{pair.Key}\": [\r\n{methodsArray}\r\n  ]";
                    jsonEntries.Add(entry);
                }

                // Correction CS1503 : Ici aussi, on applique .ToArray()
                string finalJson = "{\r\n" + string.Join(",\r\n", jsonEntries.ToArray()) + "\r\n}";
                System.IO.File.WriteAllText(this.jsonFilePath, finalJson);
                Logger.Info($"[JSON SYSTEM] Fichier mis à jour avec succès : {this.jsonFilePath}");
            }
            catch (Exception ex)
            {
                Logger.Error("[JSON SYSTEM] Erreur lors de la sauvegarde du fichier : " + ex.Message);
            }
        }

        private string GetMethodSignature(System.Reflection.MethodInfo method)
        {
            // Type de retour (ex: Void, Boolean, Int32...)
            ...
            {
                // Format : "Type NomParametre" (ex: "Boolean newTargetValue")
                paramStrings.Add($"{p.ParameterType.Name} {p.Name}");
            }

            string paramsJoined = string.Join(", ", paramStrings.ToArray());

            // Exemple de résultat : "Void SetTarget(Boolean newTargetValue)"
            return $"{returnType} {method.Name}({paramsJoined})";
        }
